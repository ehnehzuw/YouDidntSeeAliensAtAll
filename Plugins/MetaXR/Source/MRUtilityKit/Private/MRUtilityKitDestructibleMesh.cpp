...
void UMRUKDestructibleMeshComponent::SegmentMesh(const TArray<FVector>& MeshPositions, const TArray<uint32>& MeshIndices, float PointsPerUnitX, float PointsPerUnitY, float PointsPerUnitZ)
{
	...
	UMRUKBPLibrary::CreateMeshSegmentation(
		MeshPositions,
		MeshIndices,
		PointsPerUnitX,
		PointsPerUnitY,
	...
	GenerateMeshSegments(Segments, ReservedMeshSegment);
	OnMeshesGenerated.Broadcast();
}

void UMRUKDestructibleMeshComponent::GenerateMeshSegments(const TArray<FMRUKMeshSegment>& MeshSegments, const FMRUKMeshSegment& ReservedMeshSegment)
{
